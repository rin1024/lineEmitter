...
float map( vec2 p ){
    ...
    float f = texture2D( tex, p/ resolution.xy, -100.0 ).x;
    
    ///f *= sqrt( texture2D( iChannel2, (0.03*p + 2.0*gAnimTime)/256.0 ).x );
    f *= sqrt( texture2D( tex, (0.03*p + 2.0*gAnimTime)/256.0 ).x );
...
vec3 render( in vec3 ro, in vec3 rd ){
    ...
    if( t>0.0 )
    {
        ...
        vec3 _tex = texcyl( tex, 0.2*uvw + 13.1*hash1(id), nor ).xyz;
        
        mate *= 0.2+4.0*pow( desat(_tex,0.3), vec3(2.0) );
        ///mate *= 1.5*sqrt(texture2D( iChannel0, pos.xz/iChannelResolution[0].xy ).xyz);
        mate *= 1.5*sqrt( texture2D( tex, pos.xz/resolution.xy ).xyz );
        ...
        vec3 bn = -1.0 + 2.0*texcyl( tex, 0.2*uvw*vec3(1.0,0.2,1.0) + 13.1*hash1(id), nor ).xyz;
        ...
        col = mate * lin + _tex.x*1.5*vec3(1.0,1.0,1.0)*(0.3+0.7*rim)*spe*dif*sha;
...
void main(){
    ...
    for( int a=0; a<VIS_SAMPLES; a++ ){
        ...
        vec4 rr = vec4(1.0);//texture2D( tex, (fragcoord.xy+floor(256.0*hash2(float(a))))/resolution.xy );
        vec2 p = -1.0 + 2.0*(fragCoord.xy+rr.xz) / resolution.xy;
        p.x *= resolution.x/ resolution.y;
#if VIS_SAMPLES>3
